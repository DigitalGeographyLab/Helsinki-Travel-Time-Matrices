...
class WalkingTravelTimeMatrixComputer(BaseTravelTimeMatrixComputer):
    ...
    def run(self, departure_time):
        travel_times = None
        for variable_name, walking_speed in self.WALKING_SPEEDS:
            travel_time_matrix_computer = r5py.TravelTimeMatrixComputer(
                self.transport_network,
                origins=self.origins_destinations,
                departure=datetime.datetime.combine(self.date, departure_time),
            ...
            if travel_times is None:
                ...
                travel_times.join(_travel_times)
